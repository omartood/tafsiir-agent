...
const REFUSAL_MSG = "Ma helin tafsiir cad oo ku saabsan su’aashan gudaha xusuusta la hayo. Sidaas darteed kama jawaabi karo anigoo aan hubin.";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Setup Memvid & Embedder
    console.log("LOG: Initializing retrieval...");
    const memoryFile = path.join(process.cwd(), "data", "tafsiir_memory.mv2");
    
    if (!fs.existsSync(memoryFile)) {
        console.warn("LOG: Memory file not found, creating empty fallback context");
        return NextResponse.json({ text: "Nidaamku wali ma diyaarsana (Memory file missing). Fadlan maamulaha la xiriir." });
    }

    const embedder = getEmbedder("gemini", { apiKey, model: "gemini-embedding-001" });
    const mv = await open(memoryFile);
    ...
    const searchResult = await mv.find(message, { 
        ...
        mode: "auto",
        k: 5, 
        snippetChars: 2000 // Get enough context
    });
    ...
    // 3. Construct Context
    const context = hits.map((hit: any) => hit.snippet || "").join("\n\n---\n\n");
